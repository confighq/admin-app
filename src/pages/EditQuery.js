...
const EditQuery = () => {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/v1/query/` + query_name)
    .then(response => {
  ...
  return (
    ...
    >
      ...
      <CardActions>
        <Button variant="contained" color="primary" className={classes.createButton} onClick={() => alert('Sorry! Not implemented yet... :(')}>Update</Button>
        <Link
          component="button"
          variant="body2"
          onClick={() => clearConfig()}
